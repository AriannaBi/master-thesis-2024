...
def filter_lines(name_file):
    ...
    for index, row in df_concatenated_copy.iterrows():
        ...
        for elem in filtered_list:
            ...
            if spot.are_equivalent(filtered_list[0], elem) == False:
                array_df_to_drop.append(index)
                print("The two formulas are not equivalent [", elem, "] [", filtered_list[0], "]")
                break
    ...
    df_concatenated_copy.to_csv('output/not_equiv_simplif_all_options.txt', index=False,   header=False, sep=' ', quoting=csv.QUOTE_NONE,escapechar=' ')
    if len(array_df_to_drop) == 0:
        print("All formulas are equivalent!")
    else:
        print("The non equivalent formulas are saved in output/not_equiv_simplif_all_options.txt")
